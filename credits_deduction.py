                ...
                try:
                    ...
                    if text_input_override:
                        ...
                        if "selectors" in text_input_override:
                            # Exact element not confirmed yet — try several plausible candidates
                            # (see GRAMMARCHECK_TEXT_INPUT_SELECTORS) rather than a single guess.
                            matched_sel = fill_text_input(tool_tab, text_input_override["selectors"], text_input_override["text"])
                            if not matched_sel:
                    ...
                    # tool on live does have a Convert button).
                    auto_converts = (domain, env, tool_name) in AUTO_CONVERT_NO_BUTTON
                    if auto_converts:
                        cprint(f"  │ No convert button expected here (auto-converts on upload) — skipping click", C.DIM)
                    else:
                        ...
                        elif domain == "grammarcheck.ai":
                            # Confirmed via live DOM inspection: all 7 tools share the same
                            # .toolSubmitBtn.getRespBtn class combo, even though the id varies
                            # (#btnShadowRoot on some tools, none on others).
                            submit_candidates += SUBMIT_BTN_OVERRIDES.get(
                                (domain, tool_name), SUBMIT_BTN_OVERRIDES[(domain, "default")]
                            )
                        else:
                    ...
                    HIGH_CONFIDENCE_SELECTORS = RESULT_INDICATOR_OVERRIDES.get((domain, tool_name), []) + [
